{pkgs, lib, config, ...}: with lib; {
    ...
    programs.zsh = {
        ...
        shellAliases = {
            ...
            gitclean="git branch --merged $(git rev-parse --abbrev-ref HEAD) | grep -vE '^[* ]+(main|master|develop|release(/.*)?)$' | grep -v $(git rev-parse --abbrev-ref HEAD) | xargs -n1 git branch -d";
            hg="history | rg -N";
